...
const CanisterDetail: FC = () => {
    ...
    async function getEventLogAll() {
        const query = gql`
        query MyQuery {
            t_event_logs_v1(limit: 15, offset: ${(currentPage - 1) * 15}, order_by: {caller_time: desc}, where: {canister_id: {_eq: "${callerId}"}${orParams}${andParams}}) {
                block
                caller
                caller_time
                canister_id
    ...
    async function getEventCountAll() {
        const query = gql`
        query MyQuery {
            t_event_logs_v1_aggregate(where: {canister_id: {_eq: "${callerId}"}}) {
                aggregate {
                    count
                }
            }
    ...
    async function getEventKeys() {
        const query = gql`
            query MyQuery {
            v1_canister_event_key_group(where: {canister_id: {_eq: "${callerId}"}}, limit: 10, offset: ${(eventKeyPage - 1) * 10}) {
                canister_id
                event_key
            }
        }`
    ...
    //     input.setAttribute('value', callerId)
    //     input.select()
    ...
    return (
        <>
            <Head title={`ICES - Canister Details | ${callerId} Details`} />
            <Link
            ...
            {/* <h1 className="text-4xl font-bold mt-8">callerId: <span className="inline-block pl-5"> {callerId}</span></h1>
